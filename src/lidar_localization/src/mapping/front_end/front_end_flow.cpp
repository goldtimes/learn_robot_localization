...
bool FrontEndFlow::SaveTrajectory() {
  static std::ofstream groud_truth, laser_odom;
  static bool is_file_created = false;
  if (!is_file_created) {
    if (!FileManager::CreateDirectory(WORK_SPACE_PATH +
                                      "/slam_data/trajectory"))
      return false;
    if (!FileManager::CreateFile(
            groud_truth,
            WORK_SPACE_PATH + "/slam_data/trajectory/groud_truth.txt"))
      return false;
    if (!FileManager::CreateFile(
            laser_odom,
  ...
  for (int i = 0; i < 3; ++i) {
    for (int j = 0; j < 4; ++j) {
      groud_truth << gnss_odometry_(i, j);
      laser_odom << laser_odometry_(i, j);
      if (i == 2 && j == 3) {
        groud_truth << std::endl;
        laser_odom << std::endl;
      } else {
        groud_truth << " ";
        laser_odom << " ";
